...
func TestReserveVocUtxoChain(t *testing.T) {
	...
	for i, c := range cases {
		...
		utxos, err := m.reserveVocUtxoChain(&txbuilder.TemplateBuilder{}, "TestAccountID", c.amount, false)
...
func TestBuildVocTxChain(t *testing.T) {
	txbuilder.ChainTxUtxoNum = 3
	m := mockAccountManager(t)
	cases := []struct {
		inputUtxo  []uint64
	...
	for caseIndex, c := range cases {
		...
		tpls, gotUtxo, err := m.buildVocTxChain(utxos, acct.Signer)
		if err != nil {
			t.Fatal(err)
		}
...
func TestMergeSpendAction(t *testing.T) {
	testVOC := &bc.AssetID{}
	if err := testVOC.UnmarshalText([]byte("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")); err != nil {
		t.Fatal(err)
	}
