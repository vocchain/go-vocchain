...
import (
	...
	"github.com/voc-core/go-voc-core/metrics"
	"github.com/voc-core/go-voc-core/p2p"
	"github.com/voc-core/go-voc-core/rlp"
	"github.com/voc-core/go-voc-core/trie"
)
...
func (h *serverHandler) handle(p *peer) error {
	...
	p.Log().Debug("Light voc-core peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), h.server); err != nil {
		p.Log().Debug("Light voc-core handshake failed", "err", err)
		return err
	}
	...
	if !h.server.clientPool.connect(p, 0) {
		p.Log().Debug("Light voc-core peer registration failed", "err", errFullClientPool)
		return errFullClientPool
	}
	// Register the peer locally
	if err := h.server.peers.Register(p); err != nil {
		h.server.clientPool.disconnect(p)
		p.Log().Error("Light voc-core peer registration failed", "err", err)
		return err
	}
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light voc-core response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light voc-core message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *peer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light voc-core message arrived", "code", msg.Code, "bytes", msg.Size)
