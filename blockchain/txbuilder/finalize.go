...
func CalculateTxFee(tx *types.Tx) (fee uint64) {
	totalInputVOC := uint64(0)
	totalOutputVOC := uint64(0)
	...
	for _, input := range tx.Inputs {
		...
		if input.AssetID() == *consensus.VOCAssetID {
			totalInputVOC += input.Amount()
		}
	}

	for _, output := range tx.Outputs {
		if *output.AssetId == *consensus.VOCAssetID {
			totalOutputVOC += output.Amount
		}
	}

	fee = totalInputVOC - totalOutputVOC
	return
}
