...
// DefaultNativeAsset native VOC asset
...
func initNativeAsset() {
	...
	alias := consensus.VOCAlias

	definitionBytes, _ := serializeAssetDef(consensus.VOCDefinitionMap)
	DefaultNativeAsset = &Asset{
		Signer:            signer,
		AssetID:           *consensus.VOCAssetID,
		Alias:             &alias,
		VMVersion:         1,
		DefinitionMap:     consensus.VOCDefinitionMap,
...
// pre-define errors for supporting voc errorFormatter
var (
	...
	ErrInternalAsset  = errors.New("voc has been defined as the internal asset")
...
//Asset describe asset on voc chain
type Asset struct {
	*signers.Signer
...
func (reg *Registry) GetAliasByID(id string) string {
	//voc
	if id == consensus.VOCAssetID.String() {
		return consensus.VOCAlias
	}
...
func (reg *Registry) UpdateAssetAlias(id, newAlias string) error {
	...
	if oldAlias == consensus.VOCAlias || newAlias == consensus.VOCAlias {
