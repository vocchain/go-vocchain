...
import (
	...
	"github.com/vocchain/go-vocchain/protocol/state"
	...
	}{
		{
			txs: []*types.Tx{
				types.NewTx(types.TxData{
					...
					Outputs: []*types.TxOutput{types.NewTxOutput(*consensus.VOCAssetID, 5000, nil)},
				}),
			},
			amount: 5000,
			err:    nil,
			...
			block: &bc.Block{
				...
				Transactions: []*bc.Tx{
					types.MapTx(&types.TxData{
						...
						SerializedSize: 1,
						Inputs:         []*types.TxInput{types.NewCoinbaseInput(nil)},
						Outputs:        []*types.TxOutput{types.NewTxOutput(*consensus.VOCAssetID, 41250000000, cp)},
					}),
				},
			},
			parent: &state.BlockNode{
			...
			block: &bc.Block{
				...
				Transactions: []*bc.Tx{
					...
					types.MapTx(&types.TxData{
						Version:        1,
						SerializedSize: 1,
						Inputs:         []*types.TxInput{types.NewSpendInput([][]byte{}, *newHash(8), *consensus.VOCAssetID, 100000000, 0, cp)},
						Outputs:        []*types.TxOutput{types.NewTxOutput(*consensus.VOCAssetID, 90000000, cp)},
					}),
				},
			},
			parent: &state.BlockNode{
	...
	for i := 0; i < 100; i++ {
		block.Transactions = append(block.Transactions, types.MapTx(&types.TxData{
			...
			Inputs: []*types.TxInput{
				types.NewSpendInput([][]byte{}, *newHash(8), *consensus.VOCAssetID, 10000000000, 0, cp),
			},
			Outputs: []*types.TxOutput{
				types.NewTxOutput(*consensus.VOCAssetID, 9000000000, cp),
			},
		}))
	}
	...
	block := &bc.Block{
		...
		Transactions: []*bc.Tx{
			types.MapTx(&types.TxData{
				...
				SerializedSize: 1,
				Inputs:         []*types.TxInput{types.NewCoinbaseInput(nil)},
				Outputs:        []*types.TxOutput{types.NewTxOutput(*consensus.VOCAssetID, 41449998224, cp)},
			}),
			types.MapTx(&types.TxData{
				Version:        1,
				SerializedSize: 1,
				Inputs: []*types.TxInput{
					types.NewSpendInput([][]byte{}, *newHash(8), *consensus.VOCAssetID, 100000000, 0, cp),
					types.NewSpendInput([][]byte{}, *newHash(8), bc.AssetID{V0: 1}, 1000, 0, []byte{byte(vm.OP_FALSE)}),
				},
				Outputs: []*types.TxOutput{
					types.NewTxOutput(*consensus.VOCAssetID, 888, cp),
