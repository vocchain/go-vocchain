...
import (
	...
	"github.com/vocchain/go-vocchain/consensus/segwit"
	"github.com/vocchain/go-vocchain/errors"
	"github.com/vocchain/go-vocchain/math/checked"
...
func (g *GasState) setGas(VOCValue int64, txSize int64) error {
	if VOCValue < 0 {
		return errors.Wrap(ErrGasCalculate, "input VOC is negative")
	}

	g.VOCValue = uint64(VOCValue)

	var ok bool
	if g.GasLeft, ok = checked.DivInt64(VOCValue, consensus.VMGasRate); !ok {
...
func checkValid(vs *validationState, e bc.Entry) (err error) {
	...
	case *bc.Mux:
		...
		for assetID, amount := range parity {
			if assetID == *consensus.VOCAssetID {
		...
		for _, VOCInputID := range vs.tx.GasInputIDs {
			e, ok := vs.tx.Entries[VOCInputID]
			if !ok {
				return errors.Wrapf(bc.ErrMissingEntry, "entry for VOCCHAIN input %x not found", VOCInputID)
			}

			vs2 := *vs
			vs2.entryID = VOCInputID
...
func checkStandardTx(tx *bc.Tx, blockHeight uint64) error {
	...
	for _, id := range tx.ResultIds {
		...
		if !ok || *output.Source.Value.AssetId != *consensus.VOCAssetID {
			continue
		}
