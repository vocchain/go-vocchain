...
func isTransactionNoVocInput(tx *types.Tx) bool {
	for _, input := range tx.TxData.Inputs {
		if input.AssetID() == *consensus.VOCAssetID {
			return false
		}
	}
	return true
...
func (tp *TxPool) IsDust(tx *types.Tx) bool {
	return isTransactionNoVocInput(tx) || isTransactionZeroOutput(tx)
}
...
func (tp *TxPool) addTransaction(txD *TxDesc) error {
	...
	for _, id := range tx.ResultIds {
		...
		if !txD.StatusFail || *output.Source.Value.AssetId == *consensus.VOCAssetID {
			tp.utxo[*id] = tx
		}
	}
