...
import (
	...
	"github.com/voc-core/go-voc-core/accounts/usbwallet/trezor"
...
// or whether the voc-core app was not started on it.
...
// voc-core address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.voc-coreAddress)
	if _, err := w.trezorExchange(&trezor.voc-coreGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.voc-coreTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.voc-coreTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
