...
// Account represents an voc-core account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // voc-core account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain voc-core.ChainStateReader)
...
//   keccak256("\x19voc-core Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19voc-core Signed Message:\n%d%s", len(data), string(data))
