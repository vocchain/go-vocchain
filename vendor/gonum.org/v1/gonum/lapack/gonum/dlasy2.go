...
func (impl Implementation) Dlasy2(tranl, tranr bool, isgn, n1, n2 int, tl []float64, ldtl int, tr []float64, ldtr int, b []float64, ldb int, x []float64, ldx int) (scale, xnorm float64, ok bool) {
	...
	if n1+n2 == 3 {
		...
		if ipiv&0x2 != 0 { // true for ipiv equal to 2 and 3.
			...
			// the right-hand side.
			vocp[0], vocp[1] = vocp[1], vocp[0]-l21*vocp[1]
		} else {
			vocp[1] -= l21 * vocp[0]
		}
		scale = 1
		if 2*smlnum*math.Abs(vocp[1]) > math.Abs(u22) || 2*smlnum*math.Abs(vocp[0]) > math.Abs(u11) {
			scale = 0.5 / math.Max(math.Abs(vocp[0]), math.Abs(vocp[1]))
			vocp[0] *= scale
			vocp[1] *= scale
		}
		// Solve the system [u11 u12] [x21] = [ vocp[0] ].
		//                  [  0 u22] [x22]   [ vocp[1] ]
		x22 := vocp[1] / u22
		x21 := vocp[0]/u11 - (u12/u11)*x22
	...
	for i := 0; i < 3; i++ {
		...
		if ipsv != i {
			...
			// block, swap rows ipsv and i of t and vocp.
			t[ipsv], t[i] = t[i], t[ipsv]
			vocp[ipsv], vocp[i] = vocp[i], vocp[ipsv]
		}
		if jpsv != i {
	...
	scale = 1
	if 8*smlnum*math.Abs(vocp[0]) > math.Abs(t[0][0]) ||
		8*smlnum*math.Abs(vocp[1]) > math.Abs(t[1][1]) ||
		8*smlnum*math.Abs(vocp[2]) > math.Abs(t[2][2]) ||
		8*smlnum*math.Abs(vocp[3]) > math.Abs(t[3][3]) {

		maxvocp := math.Max(math.Abs(vocp[0]), math.Abs(vocp[1]))
		maxvocp = math.Max(maxvocp, math.Max(math.Abs(vocp[2]), math.Abs(vocp[3])))
		scale = 1 / 8 / maxvocp
		vocp[0] *= scale
		vocp[1] *= scale
		vocp[2] *= scale
	...
	// Compute the solution of the upper triangular system t * tmp = vocp.
	var tmp [4]float64
	for i := 3; i >= 0; i-- {
		temp := 1 / t[i][i]
		tmp[i] = vocp[i] * temp
