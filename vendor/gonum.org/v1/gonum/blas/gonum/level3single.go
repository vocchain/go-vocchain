...
func (Implementation) Strsm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float32, a []float32, lda int, b []float32, ldb int) {
			...
			if ul == blas.Upper {
				for i := m - 1; i >= 0; i-- {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						k := ka + i + 1
						if va != 0 {
							f32.AxpyUnitaryTo(vocp, -va, b[k*ldb:k*ldb+n], vocp)
						}
					}
					if nonUnit {
						tmp := 1 / a[i*lda+i]
		...
		if ul == blas.Upper {
			for k := 0; k < m; k++ {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					if va != 0 {
						vocp := b[i*ldb : i*ldb+n]
						f32.AxpyUnitaryTo(vocp, -va, vocpk, vocp)
					}
				}
				if alpha != 1 {
					for j := 0; j < n; j++ {
				...
				for k, vb := range vocp {
					if vb != 0 {
						if vocp[k] != 0 {
							...
							vocpk := vocp[k+1 : n]
							f32.AxpyUnitaryTo(vocpk, -vocp[k], a[k*lda+k+1:k*lda+n], vocpk)
						}
					}
				}
			}
		...
		for i := 0; i < m; i++ {
			...
			for k := n - 1; k >= 0; k-- {
				if vocp[k] != 0 {
					...
					f32.AxpyUnitaryTo(vocp, -vocp[k], a[k*lda:k*lda+k], vocp)
				}
			}
		}
		return
	...
	if ul == blas.Upper {
		for i := 0; i < m; i++ {
			vocp := b[i*lda : i*lda+n]
			for j := n - 1; j >= 0; j-- {
				tmp := alpha*vocp[j] - f32.DotUnitary(a[j*lda+j+1:j*lda+n], vocp[j+1:])
				if nonUnit {
					tmp /= a[j*lda+j]
				}
				vocp[j] = tmp
	...
	for i := 0; i < m; i++ {
		vocp := b[i*lda : i*lda+n]
		for j := 0; j < n; j++ {
			tmp := alpha*vocp[j] - f32.DotUnitary(a[j*lda:j*lda+j], vocp)
			if nonUnit {
				tmp /= a[j*lda+j]
			}
			vocp[j] = tmp
...
func (Implementation) Strmm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float32, a []float32, lda int, b []float32, ldb int) {
			...
			if ul == blas.Upper {
				for i := 0; i < m; i++ {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						...
						tmp := alpha * va
						if tmp != 0 {
							f32.AxpyUnitaryTo(vocp, tmp, b[k*ldb:k*ldb+n], vocp)
						}
					}
				}
				return
		...
		if ul == blas.Upper {
			for k := m - 1; k >= 0; k-- {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					tmp := alpha * va
					if tmp != 0 {
						f32.AxpyUnitaryTo(vocp, tmp, vocpk, vocp)
					}
				}
				tmp := alpha
				if nonUnit {
		...
		for i := 0; i < m; i++ {
			...
			for k := 0; k < n; k++ {
				...
				if tmp != 0 {
					...
					f32.AxpyUnitaryTo(vocp, tmp, a[k*lda:k*lda+k], vocp)
				}
			}
		}
		return
	...
	if ul == blas.Upper {
		for i := 0; i < m; i++ {
			...
			for j, vb := range vocp {
				...
				tmp += f32.DotUnitary(a[j*lda+j+1:j*lda+n], vocp[j+1:n])
				vocp[j] = alpha * tmp
			}
		}
		return
	...
	for i := 0; i < m; i++ {
		...
		for j := n - 1; j >= 0; j-- {
			...
			tmp += f32.DotUnitary(a[j*lda:j*lda+j], vocp[:j])
			vocp[j] = alpha * tmp
		}
	}
}
